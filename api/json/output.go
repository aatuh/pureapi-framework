...
type APIOutput[T any] struct {
	...
	Error   *apierror.DefaultAPIError `json:"error,omitempty"`
}
...
func NewJSONOutput(
	emitterLogger event.EmitterLogger, errorOrigin string,
) JSONOutput {
	if emitterLogger == nil {
		emitterLogger = event.NewNoopEmitterLogger()
	}
	return JSONOutput{
...
// handleError returns the API error if it is an *apierror.DefaultAPIError or
// an utiltypes.util. Returns InvalidOutputErrorType otherwise.
func (o JSONOutput) handleError(outputError error) *apierror.DefaultAPIError {
	if outputError == nil {
		return nil
	}
	if val, ok := outputError.(*apierror.DefaultAPIError); ok {
		return val
	}
	if val, ok := outputError.(apierror.APIError); ok {
		return apierror.APIErrorFrom(val)
	}
