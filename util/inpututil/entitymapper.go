...
func MustMapInputToEntity[Entity any](
	input any, entity *Entity, apiToDBFields APIToDBFields,
) *Entity {
...
// MapEntityToOutput maps fields from entities to an output struct
...
func MustMapEntitiesToOutput(
	entities []any, outputFactoryFn func() any, apiToDBFields APIToDBFields,
) []any {
	outputs, err := MapEntitiesToOutput(
		entities, outputFactoryFn, apiToDBFields,
	)
	if err != nil {
		panic(fmt.Errorf("MustMapEntitiesToOutput: %w", err))
	}
	return outputs
}

// MapEntitiesToOutput maps fields from entities to an output struct
// using the provided APIToDBFields. It expects the entities to be structs
...
func MapEntitiesToOutput(
	...
	for _, entity := range entities {
		...
		fmt.Println("OUTPUT ", logging.AnyToJSONString(output))
		if err != nil {
			return nil, err
		}
...
// mapEntityToOutput maps fields from an entity to an output struct
// using the provided APIToDBFields.
func mapEntityToOutput(
	entities any, output any, apiToDBFields APIToDBFields,
) error {
	entityVal := reflect.ValueOf(entities)
	if entityVal.Kind() == reflect.Ptr {
		entityVal = entityVal.Elem()
	}
	...
	// For each input.APIField, attempt to find the corresponding field in the
	// output struct.
	for apiField, DBColumn := range apiToDBFields {
		// Look for a field in the output struct whose JSON tag (or field name)
		// equals the input.APIField.APIName.
...
// input.APIFields for any struct type. For each field, if an input.APIField
// with a matching JSON name exists in allFields, that input.APIField is used.
// If the struct field hasa "validate" tag, its rules override any validations
// from the input.APIField. Additionally, if a field is tagged with
// `required:"true"`, then it must be found in allFields. It will try to find
// the "required" and "type" tags and these will also override input.APIField
// settings.
//
//	Parameters:
//		- Struct: The struct type.
//		- allFields: The input.APIFields to match against.
//
//	Returns:
//		- input.APIFields: The matched input.APIFields.
func MustMatchStructAPIFields[Struct any](
	allFields input.APIFields,
) input.APIFields {
...
func MatchStructAPIFIelds(
	t reflect.Type, allFields input.APIFields, depth int,
) input.APIFields {
	// Check if t is empty an type.
	if t.Kind() == reflect.Interface {
		panic("MatchStructAPIFIelds: type is a null interface")
	}
	// Check if t is not a struct type.
	if t.Kind() != reflect.Struct {
		panic(fmt.Sprintf(
			"MatchStructAPIFIelds: type %q is not a struct", t.Name(),
		))
	}
	...
	if depth > 100 {
		panic(fmt.Sprintf(
			"MatchStructAPIFIelds: type %q is too deep", t.Name(),
		))
	}

	// Build a lookup map from APIName to input.APIField.
	fieldsByName := make(map[string]input.APIField)
	fieldsByAlias := make(map[string]input.APIField)
	...
	for _, f := range allFields {
		if f.Alias != "" {
			...
			if _, ok := fieldsByAlias[f.Alias]; ok {
				panic(fmt.Sprintf(
					"MatchStructAPIFIelds: type %s has duplicate alias %q",
					t.Name(),
					f.Alias,
				))
			}
			...
			if _, ok := fieldsByName[f.APIName]; ok {
				panic(fmt.Sprintf(
					"MatchStructAPIFIelds: type %s has duplicate API name %q",
					t.Name(),
					f.APIName,
				))
			}
	...
	var inputFields input.APIFields
	...
	for i := range t.NumField() {
		...
		if requiredTag != "" {
			...
			if err != nil {
				panic(fmt.Sprintf(
					"MatchStructAPIFIelds: invalid value for required tag on field %s: %v",
					fieldName,
					err),
				)
			}
		...
		// Lookup the input.APIField.
		var apiField input.APIField
		var exists bool
		if useAlias {
			apiField, exists = fieldsByAlias[fieldName]
			if !exists {
				panic(fmt.Sprintf(
					"MatchStructAPIFIelds: alias field %s not found in fieldsByAlias",
					fieldName,
				))
			}
		} else {
		...
		if exists {
			...
			if extTag == "true" {
				panic(fmt.Sprintf(
					"MatchStructAPIFIelds: input.APIField not found for external field: %s",
					fieldName,
				))
			}
		...
		// Override the input.APIField.Required if the "required" tag is present.
		if requiredTag != "" {
			apiField.Required = required
		}

		// Override the input.APIField.Type if the "type" tag is present.
		...
		if fieldType.Kind() == reflect.Slice {
			...
			if elemType.Kind() == reflect.Struct {
				...
				apiField.Nested = MatchStructAPIFIelds(
					elemType, apiField.Nested, depth,
				)
			}
		} else if fieldType.Kind() == reflect.Struct {
			// Recursively process the nested struct.
			apiField.Nested = MatchStructAPIFIelds(
				fieldType, apiField.Nested, depth,
			)
		}
...
// against a slice of input.APIField definitions using a custom type map.
func validateAPIFieldTypes[Struct any](apiFields []input.APIField) error {
	var t Struct
	rt := reflect.TypeOf(t)
...
// validateAPIFieldTypesForType recursively checks that each input.APIField in
// apiFields matches the corresponding struct field in t.
func validateAPIFieldTypesForType(
	t reflect.Type, apiFields []input.APIField,
) error {
	...
	for _, apiField := range apiFields {
		...
		// If there are nested input.APIFields, expect the field to be a struct.
