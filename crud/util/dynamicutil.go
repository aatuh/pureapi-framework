...
import (
	...
	"github.com/pureapi/pureapi-framework/crud/types"
)
...
func findFieldValueByDBTag(v reflect.Value, tag string) (reflect.Value, error) {
	...
	return reflect.Value{}, fmt.Errorf("no field with db tag %q found", tag)
}

// MustMatchStructAPIFields is a generic function that returns a slice of APIFields
// for any struct type. For each field, if an APIField with a matching JSON
...
func MustMatchStructAPIFields[Struct any](allFields api.APIFields) api.APIFields {
...
func MatchStructAPIFIelds(
	t reflect.Type, allFields api.APIFields, depth int,
) api.APIFields {
	...
	fieldsByName := make(map[string]api.APIField)
	fieldsByAlias := make(map[string]api.APIField)
	...
	for i := range t.NumField() {
		...
		if fieldType.Kind() == reflect.Slice {
			...
			if elemType.Kind() == reflect.Struct {
				...
				apiField.Nested = MatchStructAPIFIelds(elemType, allFields, depth)
			}
...
func validateAPIFieldTypes[Struct any](apiFields []api.APIField) error {
	var t Struct
	rt := reflect.TypeOf(t)
...
func validateAPIFieldTypesForType(
	t reflect.Type, apiFields []api.APIField,
) error {
