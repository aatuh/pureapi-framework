...
func TestOrderDirection_StringAndMapping(t *testing.T) {
	...
	assert.Equal(t, query.OrderAsc, DirectionsToDB[DirectionAsc])
	assert.Equal(t, query.OrderAsc, DirectionsToDB[DirectionAscending])
	assert.Equal(t, query.OrderDesc, DirectionsToDB[DirectionDesc])
	assert.Equal(t, query.OrderDesc, DirectionsToDB[DirectionDescending])
}
...
func TestOrders_ToDBOrders_Success(t *testing.T) {
	...
	for _, o := range dbOrders {
		switch o.Field {
		case "col1":
			assert.Equal(t, query.OrderAsc, o.Direction)
			assert.Equal(t, "tbl", o.Table)
		case "col2":
			assert.Equal(t, query.OrderDesc, o.Direction)
			assert.Equal(t, "tbl", o.Table)
		default:
...
func TestSelectors_AddSelectorAndToDBSelectors_Success(t *testing.T) {
	...
	for _, ds := range dbSelectors {
		switch ds.Column {
		case "age":
			// "equal" should map to query.Equal.
			assert.Equal(t, query.Equal, ds.Predicate)
